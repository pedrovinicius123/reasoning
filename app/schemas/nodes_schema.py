...
class NodeSchema(SQLAlchemyAutoSchema):
    ...
    label = auto_field(validate=Length(min=1, max=128), required=True)
    desc = auto_field(validate=Length(min=30, max=1024), required=True)
    ...
    parent_edges = fields.Nested("EdgeSchema", many=True, dump_only=True)
